"""
Signing audit log — records every visual signature event including SHA-256
document hash and signer consent for ESIGN/UETA workflow evidence.
"""
...
class SignAuditService:
    ...
    def record_with_id(
        ...
        *,
        ...
        """Like record() but uses a caller-supplied audit_id (pre-generated for metadata embedding)."""
        entry: Dict[str, Any] = {
            "audit_id": audit_id,
            "event": "signature.applied",
            "at": datetime.now(timezone.utc).isoformat(),
    ...
    def get_by_id(self, audit_id: str) -> Optional[Dict[str, Any]]:
        """Return a single audit entry by its audit_id, or None if not found."""
